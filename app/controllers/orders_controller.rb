...
class OrdersController < ApplicationController
  ...
  def index
    @orders = Order.all
    @orders = @orders.only_personal_orders if params[:only_personal_orders].to_s == 'true'
    @orders = @orders.only_customer_orders if params[:only_customer_orders].to_s == 'true'
  ...
  def order_params
    params.require(:order).permit(:buyer_id, :country, :status, :city, :address, :postal_code, :only_personal_orders,
                                  :query, :product_id)
  end
end
